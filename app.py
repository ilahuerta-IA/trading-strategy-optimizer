# File: app.py (Dash - Stable Subplots, Ensure Axis Visibility)
...
# --- Figure Creation (Applies passed ranges, ensures visibility) ---
def create_dual_subplot_figure(
    ...
    yaxis1_range: Optional[List] = None,
    yaxis2_range: Optional[List] = None
) -> go.Figure:
    """Creates the Plotly figure with subplots and applies specified ranges."""
    print(f"DEBUG [create_figure]: Args - XRange: {xaxis_range}, Y1Range: {yaxis1_range}, Y2Range: {yaxis2_range}")
    ...
    # Update Layout - Apply ranges and ensure visibility settings
    fig.update_layout(
        ...
        xaxis=dict(
            ...
            range=xaxis_range, autorange=xaxis_range is None,
            rangeslider_visible=False, # <<<--- Force rangeslider off
            showticklabels=True, # <<<--- Ensure X tick labels are on
            type='date' # <<<--- Explicitly set type to date
        ),
        xaxis2=dict( # Also apply to hidden x-axis if needed
             showticklabels=False,
             rangeslider_visible=False
        ),
        yaxis=dict(
            title="Price 1", gridcolor='rgba(255, 255, 255, 0.1)', linecolor='rgba(255, 255, 255, 0.2)',
            range=yaxis1_range, autorange=yaxis1_range is None
        ),
        yaxis2=dict(
            title="Price 2", gridcolor='rgba(255, 255, 255, 0.1)', linecolor='rgba(255, 255, 255, 0.2)',
            range=yaxis2_range, autorange=yaxis2_range is None
        )
    )
    # Debug final ranges applied
    final_x = fig.layout.xaxis.range
    final_y1 = fig.layout.yaxis.range
    final_y2 = fig.layout.yaxis2.range
    print(f"DEBUG [create_figure]: Final Applied Ranges - X:{final_x}, Y1:{final_y1}, Y2:{final_y2}")
    return fig

# ==============================================================================
# Data Discovery & App Init (Keep as before)
# ==============================================================================
available_files = [] # (Keep file finding logic as before)
...
# ==============================================================================
# App Layout (Keep as before - remove X controls)
# ==============================================================================
...
@app.callback(
    ...
    # Removed State('bars-input', 'value')
)
def create_figure_on_asset_change(selected_file_1: str, selected_file_2: str) -> go.Figure:
    ...
    _data_cache.clear() # Clear cache for refresh

    # --- Load Data ---
    df1, df2 = None, None
    title_1 = "Select Asset 1"
    title_2 = "Select Asset 2"
    try: # Wrap data loading
        if selected_file_1:
        ...
        return create_dual_subplot_figure(None, "Error Loading Data", None, "Error Loading Data")
    ...
    if available_dfs:
        try:
            ...
            num_bars = DEFAULT_BARS_TO_SHOW # Use fixed default
    ...
    print(f"DEBUG [create_figure_on_asset_change]: Calculated initial Y-Ranges: Y1={yaxis_range1}, Y2={yaxis_range2}")

    # --- Create the final figure with ALL calculated ranges ---
    try:
        fig = create_dual_subplot_figure(
            df1, title_1, df2, title_2,
            xaxis_range=initial_xaxis_range,
            yaxis1_range=yaxis_range1,
            yaxis2_range=yaxis_range2
        )
...
def adjust_y_zoom_subplot(n_in: int, n_out: int, n_reset: int, target_chart: int, current_figure_state: Dict) -> Any:
    ...
    for chart_index in charts_to_update:
        ...
        current_y_range = current_axis.get('range'); new_y_range = None
        if button_id == 'y-zoom-reset-button': update_needed = True; print(f"DEBUG [Y-Zoom]: Resetting {axis_name}")
            ...
            if np.isclose(range_height, 0): print(f"DEBUG [Y-Zoom]: Warn - {axis_name} range is zero."); continue
            ...
            if new_y_range and new_y_range[0] < new_y_range[1]: update_needed = True; print(f"DEBUG [Y-Zoom]: Zooming {axis_name}: {new_y_range}")
            else: print(f"DEBUG [Y-Zoom]: Warn - Invalid Y range calc for {axis_name}, resetting."); new_y_range = None; update_needed = True
        elif button_id != 'y-zoom-reset-button': print(f"DEBUG [Y-Zoom]: Cannot calc zoom for {axis_name} (autorange?).")
