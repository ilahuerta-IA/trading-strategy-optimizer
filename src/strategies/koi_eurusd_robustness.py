"""KOI EURUSD Robustness Test - 10 Date Ranges
==============================================
...
BASELINE = {
    ...
    'sharpe': 3.03,
    'yearly': {
        2020: {'trades': 40, 'wr': 32.5, 'pf': 1.33, 'pnl': 4128},
        2021: {'trades': 19, 'wr': 31.6, 'pf': 1.31, 'pnl': 1858},
        2022: {'trades': 41, 'wr': 26.8, 'pf': 1.03, 'pnl': 525},
        2023: {'trades': 30, 'wr': 46.7, 'pf': 2.40, 'pnl': 12622},
        2024: {'trades': 9, 'wr': 33.3, 'pf': 1.42, 'pnl': 1434},
        2025: {'trades': 34, 'wr': 41.2, 'pf': 1.96, 'pnl': 11569},
    }
}
...
def run_backtest(from_date: str, to_date: str) -> dict:
    ...
    # Read strategy file
    strategy_file = Path(__file__).parent / 'koi_eurusd_pro.py'
    ...
    # Write temp file
    temp_file = Path(__file__).parent / '_temp_robustness.py'
...
def main():
    print("=" * 100)
    print("KOI EURUSD ROBUSTNESS TEST - 10 Date Ranges")
    print("=" * 100)
    ...
    # 2. WR > 25% in all tests
    wr_all_pass = all(r.get('win_rate', 0) > 25 for r in valid)
    wr_check = "✅" if wr_all_pass else "❌"
    print(f"{wr_check} WR > 25% in all tests: {'YES' if wr_all_pass else 'NO'}")
    ...
    if checks_passed >= 4:
        ...
        print("⚠️ VERDICT: ACCEPTABLE ROBUSTNESS (2/4 criteria)")
    else:
