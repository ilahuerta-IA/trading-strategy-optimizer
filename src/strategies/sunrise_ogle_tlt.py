"""Advanced Sunrise Strategy - TLT ETF (Long-Only)
==================================================
Based on DIA Version - Optimized for TLT ETF Trading.
iShares 20+ Year Treasury Bond ETF - US Government Bonds.
ATR Increment/Decrement Filters Fixed/Disabled.
Short Logic Removed.

INSTRUMENT INFO (Dukascopy)
---------------------------
- Symbol: TLTUSUSD
- Point Value: 1
- Pip/Tick Size: 0.01
...
DATA_FILENAME = 'TLT_5m_5Yea.csv'     # TLT ETF Data File
...
# === FOREX CONFIGURATION ===
ENABLE_FOREX_CALC = False             # Enable advanced forex position calculations
FOREX_INSTRUMENT = 'TLT'              # Fixed to TLT
...
LONG_ATR_MIN_THRESHOLD = 0.05         
LONG_ATR_MAX_THRESHOLD = 0.21        

# ATR INCREMENT FILTER (FIXED: Disabled/Widened)
LONG_USE_ATR_INCREMENT_FILTER = False       
LONG_ATR_INCREMENT_MIN_THRESHOLD = 0.000000 
LONG_ATR_INCREMENT_MAX_THRESHOLD = 0.120000 

# ATR DECREMENT FILTER - OPTIMIZED FROM TRADE ANALYSIS (PF=2.99, WR=56.2%)
LONG_USE_ATR_DECREMENT_FILTER = False        
LONG_ATR_DECREMENT_MIN_THRESHOLD = -0.006000 
LONG_ATR_DECREMENT_MAX_THRESHOLD = -0.000500 
...
LONG_MAX_ANGLE = 65.0                       # EXPANDED: Much wider angle range for more entries
LONG_ANGLE_SCALE_FACTOR = 10.0              # TLT: Standard scale factor for bond ETF price range
...
ENTRY_START_HOUR = 14                     # Start hour for entry window (UTC)
ENTRY_START_MINUTE = 0                    # Start minute for entry window (UTC)
ENTRY_END_HOUR = 19                       # End hour for entry window (UTC)
...
class SunriseOgle(bt.Strategy):
    params = dict(
        ...
        ema_fast_length=5,              # Fast EMA period for trend detection
        ema_medium_length=7,            # Medium EMA period for trend confirmation
        ema_slow_length=9,              # Slow EMA period for trend strength
        ema_confirm_length=1,            # Confirmation EMA (usually 1 for immediate response)
        ema_filter_price_length=40,      # Price filter EMA to avoid counter-trend trades
        ...
        contract_size=1,                  # Base contract size (auto-adjusted per instrument)
        ...
        # === FOREX SETTINGS ===
        use_forex_position_calc=False,    # Enable advanced forex position calculations
        forex_instrument='TLT',           # Fixed to TLT
        forex_base_currency='USD',        # Base currency: USD
        forex_quote_currency='USD',       # Quote currency: USD
        forex_pip_value=0.01,             # Pip value for TLT (2 decimal places)
        forex_pip_decimal_places=2,       # Price decimal places for TLT
        forex_lot_size=1,                 # Lot size for TLT (1 Share)
        forex_micro_lot_size=1,           # Minimum lot increment (1 share)
        forex_spread_pips=0.0,            # Typical spread in pips for TLT
        forex_margin_required=10.0,       # Margin requirement % for TLT (10:1 leverage)

        # === ACCOUNT SETTINGS ===
        account_currency='USD',           # Account denomination currency
        account_leverage=10.0,            # Account leverage (matches broker setting)
        ...
        try:
            ...
            if self.trade_reports:
                ...
                if entry_price and exit_price:
                    pips = (exit_price - entry_price) / 0.01  # TLT pip calculation
