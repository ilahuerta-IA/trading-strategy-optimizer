"""Advanced Sunrise Strategy - NZDUSD Trading System
==================================================
NZDUSD VERSION: This is a specialized version optimized exclusively for NZDUSD (New Zealand Dollar vs USD) trading.
Adapted from the original strategy with NZDUSD-specific parameters and configurations.

This strategy implements a sophisticated trading system optimized for NZDUSD with the following features:
...
MULTI-ASSET SUPPORT
-------------------
🌏 FOREX PAIR: NZDUSD (New Zealand Dollar vs US Dollar)
   - Standard 100,000 NZD contract sizes
   - 0.0001 pip values (4 decimal places)
   - 30:1 leverage with 3.33% margin

🤖 CONFIGURATION: Instrument settings optimized for NZDUSD
   - Pip values: 0.0001
   - Lot sizes: 100,000 NZD
...
# NZDUSD version - New Zealand Dollar vs US Dollar
DATA_FILENAME = 'NZDUSD_5m_5Yea.csv'     # 🌏 New Zealand Dollar vs US Dollar - Forex Pair
...
LONG_ATR_MIN_THRESHOLD = 0.000280 #0.000280         
LONG_ATR_MAX_THRESHOLD = 0.000600 #0.000600        
...
LONG_ATR_INCREMENT_MIN_THRESHOLD = 0.000002 #0.000050 # EXPANDED: Much wider range for more entries
LONG_ATR_INCREMENT_MAX_THRESHOLD = 0.000500 # EXPANDED: Much wider range for more entries
...
LONG_ATR_DECREMENT_MIN_THRESHOLD = -0.000200 # 🎯 EXPANDED: Much wider range for more entries
LONG_ATR_DECREMENT_MAX_THRESHOLD = -0.000100 # 🎯 EXPANDED: Much wider range for more entries
...
SHORT_ANGLE_SCALE_FACTOR = 10000.0          # 🌏 NZDUSD: Standard scale factor for forex price range (4 decimal places)
...
ENTRY_START_HOUR = 19#6                      # Start hour for entry window (UTC)
ENTRY_START_MINUTE = 0                     # Start minute for entry window (UTC)
ENTRY_END_HOUR = 12#18 #15                        # End hour for entry window (UTC)
...
class SunriseOgle(bt.Strategy):
    params = dict(
        ...
        short_angle_scale_factor=SHORT_ANGLE_SCALE_FACTOR,       # Scaling factor for angle calculation sensitivity (short entries - 10000.0 for NZDUSD)
    ...
    def _calculate_forex_position_size(self, entry_price, stop_loss_price):
        ...
        # Calculate value per tick for NZDUSD
        # For NZDUSD: 1 standard lot (100,000 NZD) = $10 per pip (0.0001 price move)
        
        if self.p.forex_quote_currency == 'USD':
            pip_value_per_lot = 10.0  # Standard forex: $10 per pip for 100K lot
        else:
            # For non-USD quote, would need conversion rate
        ...
        # Convert to Backtrader contracts for NZDUSD
        # For NZDUSD: Standard forex lot sizing (100,000 NZD per lot)
    ...
    def _validate_forex_setup(self):
        """Validate forex configuration for NZDUSD.
        
        Returns:
            bool: True if configuration is valid for NZDUSD data
        """
        if not self.p.use_forex_position_calc:
            return True
    ...
    def _get_forex_instrument_config(self, instrument_name=None):
        """Get configuration for NZDUSD instrument.
        
        Args:
            instrument_name: Override instrument name (defaults to NZDUSD)
            
        Returns:
            dict: Configuration dictionary for NZDUSD
        """
    ...
    def _apply_forex_config(self):
        """Apply forex configuration for NZDUSD."""
        if not self.p.use_forex_position_calc:
            return
