"""OGLE-KOI Dual Strategy - TEMPLATE for New Assets
===================================================
TEMPLATE VERSION: Copy and customize for each new forex pair
...
CUSTOMIZATION STEPS:
1. Copy this file as oglekoi_{asset}.py
2. Update DATA_FILENAME, FOREX_INSTRUMENT, PIP_VALUE
3. Import correct OGLE strategy (sunrise_ogle_{asset}_pro.py)
4. Adjust KOI parameters if needed for the asset
5. Run and optimize

STRATEGY 1 - KOI:
- Bullish Engulfing + 5 EMAs ascending + CCI + Breakout Window
- Parameters vary by asset (see koi_{asset}_pro.py)

STRATEGY 2 - OGLE:
- EMA Crossover + Pullback + Volatility Expansion Channel
- Parameters vary by asset (see sunrise_ogle_{asset}_pro.py)
...
# =============================================================================
# *** CUSTOMIZE THIS SECTION FOR EACH ASSET ***
# =============================================================================

# Asset Configuration - CHANGE THESE FOR EACH PAIR
DATA_FILENAME = 'XXXYYY_5m_5Yea.csv'  # CHANGE: e.g., 'EURUSD_5m_5Yea.csv'
FOREX_INSTRUMENT = 'XXXYYY'            # CHANGE: e.g., 'EURUSD'
PIP_VALUE = 0.0001                     # CHANGE: 0.0001 for most pairs, 0.01 for JPY pairs

# OGLE Strategy Import - CHANGE TO MATCH ASSET
OGLE_STRATEGY_MODULE = 'sunrise_ogle_xxxyyy_pro'  # CHANGE: e.g., 'sunrise_ogle_eurusd_pro'
...
# =============================================================================
# *** KOI PARAMETERS - CUSTOMIZE PER ASSET ***
# =============================================================================

# EMAs (usually same across assets)
KOI_EMA_PERIODS = [10, 20, 40, 80, 120]

# CCI - adjust threshold per asset
KOI_CCI_PERIOD = 20
KOI_CCI_THRESHOLD = 100  # CUSTOMIZE: 100-120 typical range

# ATR SL/TP - adjust multipliers per asset
KOI_ATR_LENGTH = 10
KOI_ATR_SL_MULT = 2.5    # CUSTOMIZE: 2.0-3.0 range
KOI_ATR_TP_MULT = 7.5    # CUSTOMIZE: 6.0-12.0 range

# Breakout Window
KOI_BREAKOUT_WINDOW = 3
KOI_BREAKOUT_OFFSET_PIPS = 2.0

# SL Filter - adjust per asset volatility
KOI_MIN_SL_PIPS = 8.0    # CUSTOMIZE
KOI_MAX_SL_PIPS = 15.0   # CUSTOMIZE

# ATR Filter - adjust per asset volatility
KOI_USE_ATR_FILTER = True
KOI_ATR_MIN = 0.00040    # CUSTOMIZE: min ATR threshold
KOI_ATR_MAX = 0.00100    # CUSTOMIZE: max ATR threshold

# Session Filter - adjust per asset
KOI_USE_SESSION_FILTER = True
KOI_PROFITABLE_HOURS = [0, 4, 5, 7, 8, 10, 11, 12, 13, 14, 16, 18, 22, 23]  # CUSTOMIZE


# =============================================================================
# CUSTOM CSV DATA FEED - Handles Date/Time separate columns
# =============================================================================
...
# =============================================================================
# KOI STRATEGY - TEMPLATE (customize params section)
# =============================================================================
class KOIStrategy(bt.Strategy):
    """KOI: Bullish Engulfing + 5 EMAs + CCI + Breakout Window"""
    
    params = dict(
        ema_periods=KOI_EMA_PERIODS,
        cci_period=KOI_CCI_PERIOD,
        cci_threshold=KOI_CCI_THRESHOLD,
        atr_length=KOI_ATR_LENGTH,
        atr_sl_mult=KOI_ATR_SL_MULT,
        atr_tp_mult=KOI_ATR_TP_MULT,
        breakout_window=KOI_BREAKOUT_WINDOW,
        breakout_offset_pips=KOI_BREAKOUT_OFFSET_PIPS,
        min_sl_pips=KOI_MIN_SL_PIPS,
        max_sl_pips=KOI_MAX_SL_PIPS,
        use_atr_filter=KOI_USE_ATR_FILTER,
        atr_min_threshold=KOI_ATR_MIN,
        atr_max_threshold=KOI_ATR_MAX,
        use_session_filter=KOI_USE_SESSION_FILTER,
        profitable_hours=KOI_PROFITABLE_HOURS,
    ...
    def _get_datetime(self, offset=0):
        try:
            return bt.num2date(self.data.datetime[offset])
        except Exception:
            return datetime.now()
    ...
    def _check_session(self, dt):
        if not self.p.use_session_filter:
            return True
        return dt.hour in self.p.profitable_hours
    ...
    def next(self):
        ...
        self._timestamps.append(self._get_datetime())
...
# =============================================================================
# OGLE STRATEGY WRAPPER - Import based on asset
# =============================================================================
BASE_DIR = Path(__file__).resolve().parent
sys.path.insert(0, str(BASE_DIR))

try:
    # Dynamic import based on OGLE_STRATEGY_MODULE
    ogle_module = __import__(OGLE_STRATEGY_MODULE)
    OGLEStrategyBase = ogle_module.SunriseOgle
    OGLE_AVAILABLE = True
    print(f"[OK] OGLE strategy imported from {OGLE_STRATEGY_MODULE}")
except ImportError as e:
    print(f"[WARN] Could not import OGLE strategy from {OGLE_STRATEGY_MODULE}: {e}")
...
def create_data_feed(fromdate, todate):
    """Create Backtrader data feed from CSV file using custom ForexCSVData."""
...
def aggregate_results(results_list, starting_cash):
    ...
    print(f"\n" + "=" * 70)
    print(f"=== OGLE-KOI DUAL STRATEGY SUMMARY ({FOREX_INSTRUMENT}) ===")
    print(f"=" * 70)
    ...
    # =================================================================
    # REALISTIC RATIO CALCULATIONS
    # =================================================================
    # Trade-based returns (realistic for low-frequency strategies)
    trade_returns = []
    if all_trade_pnls:
    ...
    sharpe_ratio = 0.0
    trades_per_year = 0
    if len(trade_returns) > 10 and all_trade_pnls:
    ...
    if trades_per_year > 0:
        print(f"Trades/Year: {trades_per_year:.1f} (annualization factor: √{trades_per_year:.0f})")
    
    sharpe_status = "Poor" if sharpe_ratio < 0.5 else "Marginal" if sharpe_ratio < 1.0 else "Good" if sharpe_ratio < 2.0 else "Excellent"
    print(f"Sharpe Ratio (Trade):  {sharpe_ratio:>8.2f}  [{sharpe_status}]")
    ...
    print(f"Sortino Ratio (Trade): {sortino_ratio:>8.2f}  [{sortino_status}]")
...
def create_portfolio_charts(results_list, starting_cash):
    """Create interactive portfolio charts with 2 subplots."""
    if not ENABLE_PLOT:
        return
    ...
    def on_hover(event):
        ...
        if event.inaxes in [ax1, ax2] and event.xdata and event.ydata:
            if interactive_text:
                interactive_text.remove()
            date_str = mdates.num2date(event.xdata).strftime('%Y-%m-%d %H:%M')
            value_str = f'${event.ydata:,.0f}'
            subplot_name = "Individual" if event.inaxes == ax1 else "Combined"
            interactive_text = fig.text(0.5, 0.5, f'{subplot_name} | Date: {date_str} | Value: {value_str}',
