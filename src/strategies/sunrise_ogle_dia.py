"""Advanced Sunrise Strategy - DIA ETF (Long-Only)
==================================================
Based on NZDUSD Version - Optimized for DIA ETF Trading.
...
DATA_FILENAME = 'DIA_5m_5Yea.csv'     # Default Data File
...
FOREX_INSTRUMENT = 'DIA'              # Fixed to DIA
...
LONG_ANGLE_SCALE_FACTOR = 10.0              # 🌏 DIA: Standard scale factor for stock price range
...
class SunriseOgle(bt.Strategy):
    params = dict(
        ...
        forex_pip_value=0.01,             # Pip value for DIA (2 decimal places)
        forex_pip_decimal_places=2,       # Price decimal places for DIA
        forex_lot_size=1,                 # Lot size for DIA (1 Share)
        forex_micro_lot_size=1,           # Minimum lot increment (1 share)
        forex_spread_pips=0.0,            # Typical spread in pips for DIA
        forex_margin_required=10.0,       # Margin requirement % for DIA (10:1 leverage)
        ...
        try:
            ...
            if self.trade_reports:
                ...
                if entry_price and exit_price:
                    pips = (exit_price - entry_price) / 0.01  # DIA pip calculation
