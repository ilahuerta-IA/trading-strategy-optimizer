"""OGLE-KOI Dual Strategy - Combined Backtest for EURUSD
=======================================================
ASSET: EURUSD 5M
TIMEFRAME: 5 minutes
...
STRATEGY 1 - KOI (koi_eurusd_pro.py optimized):
- Bullish Engulfing + 5 EMAs ascending + CCI > 110 + Breakout Window
- SL: 2.0x ATR, TP: 6.0x ATR (RR 1:3)
- SL Filter: 8-14 pips
- ATR Filter: 50-100 pips
- Session Filter: [0, 4, 5, 7, 8, 10, 11, 12, 13, 14, 16, 18, 22, 23] UTC
- Performance: PF 1.54, 173 trades, +$32,149

STRATEGY 2 - OGLE (sunrise_ogle_eurusd_pro.py optimized):
- EMA Crossover + Pullback + Volatility Expansion Channel
- SL: 3.0x ATR, TP: 15.0x ATR
- ATR Filter: 20-40 pips
- Time Filter: 22:00-08:00 UTC
- EMAs: 24/24/24/1/60
...
# =============================================================================
# KOI STRATEGY - EURUSD OPTIMIZED (from koi_eurusd_pro.py)
# =============================================================================
class KOIStrategy(bt.Strategy):
    """KOI EURUSD: Bullish Engulfing + 5 EMAs + CCI + Breakout Window
    
    OPTIMIZED PARAMETERS (PF 1.54, 173 trades):
    - CCI threshold: 110
    - ATR SL/TP: 2.0x / 6.0x (RR 1:3)
    - SL Filter: 8-14 pips
    - ATR Filter: 50-100 pips
    - Session Filter: Profitable hours only
    """
    ...
    params = dict(
        ...
        # CCI
        cci_period=20,
        cci_threshold=110,  # EURUSD optimized
        # ATR
        atr_length=10,
        atr_sl_mult=2.0,    # EURUSD optimized
        atr_tp_mult=6.0,    # EURUSD optimized (RR 1:3)
        # Breakout window
        breakout_window=3,
        breakout_offset_pips=2.0,
        # SL Filter
        min_sl_pips=8.0,    # EURUSD optimized
        max_sl_pips=14.0,   # EURUSD optimized
        # ATR Filter
        use_atr_filter=True,
    ...
    def _check_session(self, dt):
        """Check if current hour is in profitable hours."""
...
# =============================================================================
# OGLE STRATEGY WRAPPER - Imports actual EURUSD strategy
# =============================================================================
BASE_DIR = Path(__file__).resolve().parent
sys.path.insert(0, str(BASE_DIR))

try:
    from sunrise_ogle_eurusd_pro import SunriseOgle as OGLEStrategyBase
    OGLE_AVAILABLE = True
    print("[OK] OGLE EURUSD strategy imported successfully")
except ImportError as e:
    print(f"[WARN] Could not import OGLE EURUSD strategy: {e}")
...
def aggregate_results(results_list, starting_cash):
    ...
    print(f"\n" + "=" * 70)
    print(f"=== OGLE-KOI DUAL STRATEGY SUMMARY (EURUSD) ===")
    print(f"=" * 70)
    ...
    # =================================================================
    # Note: Sharpe/Sortino based on TRADE returns (not daily equity curve)
    # This is more realistic for low-frequency strategies where most days
    # have no trades. Using daily equity curve would artificially inflate
    # ratios due to many zero-return days.
    
    # Trade-based returns (realistic for this strategy frequency)
    trade_returns = []
    if all_trade_pnls:
    ...
    if len(trade_returns) > 10 and all_trade_pnls:
        ...
        if std_return > 0:
            # Annualize based on actual trades per year
    ...
    # Show trades/year for context
    if all_trade_pnls:
        first_date = min(t['date'] for t in all_trade_pnls)
        last_date = max(t['date'] for t in all_trade_pnls)
        years = max((last_date - first_date).days / 365.25, 0.1)
        trades_per_year = len(all_trade_pnls) / years
...
def create_portfolio_charts(results_list, starting_cash):
    ...
    - Top: Individual strategies (KOI and OGLE) superimposed
    - Bottom: Combined portfolio (sum of both strategies)
    """
    if not ENABLE_PLOT:
        return
    ...
    fig.suptitle('OGLE-KOI Multi-Strategy Backtest (EURUSD 5M)', 
                 fontsize=20, fontweight='bold', y=0.98)
    
    # =================================================================
    # SUBPLOT 1: INDIVIDUAL STRATEGIES (KOI & OGLE superimposed)
    # =================================================================
    print(f"  [PLOT] Subplot 1: Individual Strategies (KOI & OGLE)")
    ...
    # Title with individual performance
    title_parts = []
    for name, data in portfolio_data.items():
        if data['values']:
            initial = data['initial_value']
    ...
    # =================================================================
    print(f"  [PLOT] Subplot 2: Combined Portfolio")
    ...
    # =================================================================
    # INTERACTIVE HOVER (works on both subplots)
    # =================================================================
    ...
    print(f"  [TIP] Move mouse over chart to see data points")
...
def run_dual_strategy_backtest():
    ...
    print(f"=== OGLE-KOI DUAL STRATEGY BACKTEST (EURUSD) ===")
