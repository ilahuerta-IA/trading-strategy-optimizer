...
==================================================
Based on USDCAD Version - Optimized for Long-Only Trading.
...
🎯 PRIMARY: ATR-based Stop Loss & Take Profit (OCA orders)
   - LONG: Stop Loss = entry_bar_low - (ATR × SL_Mult), Take Profit = entry_bar_high + (ATR × TP_Mult)
...
DATA_FILENAME = 'USDCAD_5m_5Yea.csv'     # Default Data File
...
FOREX_INSTRUMENT = 'USDCAD'           # Fixed to USDCAD (USD vs Canadian Dollar)
...
LONG_ATR_MAX_THRESHOLD = 0.00300        

# ATR INCREMENT FILTER (FIXED: Disabled/Widened)
LONG_USE_ATR_INCREMENT_FILTER = False       
LONG_ATR_INCREMENT_MIN_THRESHOLD = 0.000002 
LONG_ATR_INCREMENT_MAX_THRESHOLD = 0.000050 
...
LONG_ANGLE_SCALE_FACTOR = 10000.0           # 🌏 USDCAD: Standard scale factor for forex price range (4 decimal places)
...
ENTRY_START_HOUR = 17                     # Start hour for entry window (UTC)
ENTRY_START_MINUTE = 0                    # Start minute for entry window (UTC)
ENTRY_END_HOUR = 10                       # End hour for entry window (UTC)
...
class SunriseOgle(bt.Strategy):
    params = dict(
        ...
        ema_fast_length=9,              # Fast EMA period for trend detection
        ema_medium_length=11,            # Medium EMA period for trend confirmation
        ...
        long_angle_scale_factor=LONG_ANGLE_SCALE_FACTOR,       # Scaling factor for angle calculation sensitivity (long entries - 10000.0 for USDCAD)
        ...
        forex_instrument='USDCAD',        # Fixed to USDCAD
        forex_base_currency='USD',        # Base currency: USD
        forex_quote_currency='CAD',       # Quote currency: CAD
        forex_pip_value=0.0001,           # Pip value for USDCAD (4 decimal places)
        forex_pip_decimal_places=4,       # Price decimal places for USDCAD
        forex_lot_size=100000,            # Lot size for USDCAD (100K USD)
        forex_micro_lot_size=0.01,        # Minimum lot increment (0.01 standard lots)
        forex_spread_pips=2.0,            # Typical spread in pips for USDCAD
        forex_margin_required=2.0,        # Margin requirement % for USDCAD (50:1 leverage approx, or 3.33 for 30:1)
    ...
    def _calculate_forex_position_size(self, entry_price, stop_loss_price):
        ...
        contracts = max(1, int(optimal_lots * 100000))  # Scale lots to USD units
    ...
    def _format_forex_trade_info(self, entry_price, stop_loss, take_profit, lot_size, pip_risk, position_value, margin_required):
        ...
        # Calculate monetary values for USDCAD
        # Standard forex: $10 per pip for 100K lot (approximate for non-USD quote)
        pip_value_per_lot = 10.0
    ...
    def _validate_forex_setup(self):
        ...
        # Check if data filename matches USDCAD
        data_filename = getattr(self, '_data_filename', '')
        if 'USDCAD' not in data_filename.upper():
            print(f"WARNING: Data file '{data_filename}' may not be USDCAD data")
            
        # Validate price ranges for USDCAD (typical range 1.20-1.46)
        if hasattr(self.data, 'close') and len(self.data.close) > 0:
            current_price = float(self.data.close[0])
            if not (1.00 < current_price < 1.60):
                print(f"WARNING: USDCAD price {current_price:.5f} outside typical range (1.00-1.60)")
                
        # Check tick value consistency for USDCAD
        if self.p.forex_pip_value != 0.0001:
            print(f"WARNING: USDCAD pip value should be 0.0001, currently {self.p.forex_pip_value}")
    ...
    def _get_forex_instrument_config(self, instrument_name=None):
        ...
        if instrument_name is None or instrument_name == 'AUTO':
            ...
            if 'USDCAD' in data_filename:
                instrument_name = 'USDCAD'
            elif 'NZDUSD' in data_filename:
                instrument_name = 'NZDUSD'
            else:
                instrument_name = 'USDCAD'  # Default to USDCAD
        ...
        config = {
            ...
            'USDCAD': {
                ...
                'pip_value': 0.0001,         # Standard 4-decimal pip
                'pip_decimal_places': 4,
                'lot_size': 100000,          # 100K USD standard lot
                'margin_required': 2.0,      # 50:1 leverage (or 3.33 for 30:1)
                'typical_spread': 2.0        # ~2 pips typical spread
            }
        }
        
        return config.get(instrument_name, config['USDCAD'])
    
    def _apply_forex_config(self):
        if not self.p.use_forex_position_calc:
            return
            
        # Get configuration for USDCAD
        config = self._get_forex_instrument_config('USDCAD')
        
        # Update parameters with USDCAD configuration
        ...
        self._detected_instrument = 'USDCAD'
        data_filename = getattr(self, '_data_filename', '').upper()
                
        # Apply USDCAD configuration
        ...
        # Update the instrument parameter with USDCAD
        self.p.forex_instrument = 'USDCAD'
                
        # Log forex configuration
        print(f"CONFIGURED: USDCAD from filename: {data_filename}")
...
if __name__ == '__main__':
    ...
    if ENABLE_FOREX_CALC:
        ...
        print(f">> Instrument: USDCAD (USD/CAD)")
    else:
